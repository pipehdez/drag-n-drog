...
export default function useWebCamSensor(api) {
  const start = useCallback(
    function start() {
      const videoInput = document.getElementById("videoInput"); // video is the 
      const src = new cv.Mat(videoInput.height, videoInput.width, window.cv.CV_8UC4);
      const cap = new cv.VideoCapture(videoInput);
      setTimeout(() => {
        const initialCoordinates = processFirstFrame(src, cap);
        if (!initialCoordinates) return;
        const preDrag = api.tryGetLock(initialCoordinates.x < 260 ? "11" : "5");
